// src/pages/regional/RegionalQuoteOperations.jsx
...
export default function RegionalQuoteOperations() {
  ...
  const fetchOfficeSettings = useCallback(async () => {
    ...
    setSettingsLoading(false);
  }, [supabaseClient, profile?.role, profile?.region]);
  ...
  const fetchDeviceTokens = useCallback(async ({ quiet = false } = {}) => {
    ...
    if (tokenError) {
      console.error('[RegionalQuoteOperations] extension device query failed:', tokenError);
      if (!quiet) {
  ...
  const fetchQuotes = useCallback(
    async ({ quiet = false } = {}) => {
      ...
      // Wait for office_dashboard_settings to resolve before querying quote_log_view.
      if (settingsLoading) {
        if (!quiet) setLoading(false);
        return;
      }
      ...
      for (let index = 0; index < quoteIds.length; index += 200) {
        ...
        if (quoteError) {
          console.error('[RegionalQuoteOperations] completeness query failed:', quoteError);
        } else {
          supplementalRows.push(...(quoteData || []));
        }

        if (eventsError) {
          console.error('[RegionalQuoteOperations] quote events query failed:', eventsError);
        } else {
          eventRows.push(...(eventsData || []));
        }

        if (workflowError) {
          console.error('[RegionalQuoteOperations] 3 Yes workflow query failed:', workflowError);
        } else {
  ...
  const isRegional = cleanStr(profile?.role).toLowerCase() === 'regional';
  ...
  const regions = useMemo(() => {
    ...
    customerGroups.forEach((group) => values.add(normalizeRegion(group.region) || 'UNASSIGNED'));
  ...
  const offices = useMemo(() => {
    ...
    Object.entries(officeRegions).forEach(([office, region]) => {
      if (regionFilter === 'all' || normalizeRegion(region) === normalizeRegion(regionFilter)) values.add(normalizeOffice(office));
    });

    customerGroups.forEach((group) => {
      if (regionFilter === 'all' || normalizeRegion(group.region) === normalizeRegion(regionFilter)) values.add(group.office);
    });
  ...
  const agents = useMemo(() => {
    const values = new Set();
    customerGroups.forEach((group) => {
      if (regionFilter !== 'all' && normalizeRegion(group.region) !== normalizeRegion(regionFilter)) return;
  ...
  const filteredDeviceTokens = useMemo(() => {
    ...
    return deviceTokens.filter((row) => {
      if (regionFilter !== 'all' && normalizeRegion(row.region || 'UNASSIGNED') !== normalizeRegion(regionFilter)) return false;
  ...
  const saveCustomerOutcome = async (event) => {
    ...
    if (saveError) {
      console.error('[RegionalQuoteOperations] workflow save failed:', saveError);
    ...
    if (overrideHistoryError) {
      console.error('[RegionalQuoteOperations] override history save failed:', overrideHistoryError);
    ...
    if (agentEntry.notes.trim()) {
      ...
      if (noteError) {
        console.error('[RegionalQuoteOperations] internal note save failed:', noteError);
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          <p className={styles.eyebrow}>Regional / Region View</p>
          <h1>Quote Operations</h1>
          <p className={styles.subtitle}>
            Manage your region's live quotes, review Lost Deals and Walks, track existing-client re-writes, and confirm closed business by office and agent.
          </p>
        </div>
