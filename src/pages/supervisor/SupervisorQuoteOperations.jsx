// src/pages/supervisor/SupervisorQuoteOperations.jsx
...
const ALL_NOT_CLOSED_REASONS = [...NOT_CLOSED_REASONS, ...LEGACY_NOT_CLOSED_REASONS];
...
function contactMethodLabel(quote) {
  const method = cleanStr(quote?.contact_method).toLowerCase();
  if (method === 'in_person') return 'In Person';
  if (method === 'phone') return 'Over the Phone';
...
function customerOutcomeFromQuotes(quotes) {
  ...
  // A new active attempt must not inherit an older Lost Deal / Walk / Follow-Up
  // disposition from the same customer. The newest quote attempt is authoritative.
  const latest = sorted[0] || null;
  const latestOutcome = cleanStr(latest?.outcome).toLowerCase();
  const latestHasDisposition = Boolean(
    latestOutcome === 'sold' ||
    latestOutcome === 'follow_up' ||
    ALL_NOT_CLOSED_REASONS.some(([value]) => value === latestOutcome) ||
  ...
  if (latest && getDisplayStatus(latest) === 'in_progress' && !latestHasDisposition) {
    return {
      status: 'open',
      source: latest,
      reason: sorted.slice(1).some((q) => q.outcome === 'lost_deal')
        ? 'Re-opened Lost Deal'
        : 'Open',
      reopenedLostDeal: sorted.slice(1).some((q) => q.outcome === 'lost_deal'),
    };
  }

  // Workflow disposition is authoritative even when TurboRater never bridged back.
  // This lets admin/supervisor close, not-close, or follow-up an in-progress quote
  // and immediately removes it from Live Activity.
  const sold = sorted.find((q) => q.outcome === 'sold');
  if (sold) return { status: 'closed', source: sold, reason: 'Closed / Policy Sold' };

  const followUpOverride = sorted.find((q) => q.outcome === 'follow_up' || q.follow_up_needed);
  if (followUpOverride) {
    return {
      status: 'follow_up',
      source: followUpOverride,
      reason: 'Follow Up',
      followUpAt: followUpOverride.follow_up_at || null,
    };
  }

  const notClosedOverride = sorted.find((q) =>
    ALL_NOT_CLOSED_REASONS.map(([value]) => value).includes(q.outcome)
  );
  if (notClosedOverride) {
    return {
      status: 'not_closed',
      source: notClosedOverride,
      reason: reasonLabel(notClosedOverride.outcome),
    };
  }
  ...
  return {
    status: 'open',
    source: completed[0] || sorted[0] || null,
    reason: 'Open',
  };
}
    ...
    .map((group) => {
      ...
      return {
        ...
        lastActivityAt:
          ...
          latest?.created_at,
        outcome,
        reopenedLostDeal: Boolean(outcome?.reopenedLostDeal),
        yesStage: Math.max(0, ...sorted.map(threeYesStage)),
        stageQuote: sorted.find((q) => threeYesStage(q) === Math.max(0, ...sorted.map(threeYesStage))) || latest,
...
export default function SupervisorQuoteOperations() {
  ...
  const officeAccessRefreshTimer = useRef(null);
  const [officeDevices, setOfficeDevices] = useState([]);
  const [officeDevicesLoading, setOfficeDevicesLoading] = useState(false);
  const [officeDeviceMessage, setOfficeDeviceMessage] = useState('');
  ...
  const fetchOfficeSettings = useCallback(async () => {
    ...
    if (settingsError) {
      console.error('[SupervisorQuoteOperations] office settings query failed:', settingsError);
  ...
  const fetchOfficeDevices = useCallback(async ({ quiet = false } = {}) => {
    if (!supabaseClient || userRole !== 'supervisor') return;

    if (!quiet) {
      setOfficeDevicesLoading(true);
      setOfficeDeviceMessage('');
    }

    const { data, error: deviceError } = await supabaseClient
      .rpc('get_supervisor_office_devices');

    if (deviceError) {
      console.error('[SupervisorQuoteOperations] office device query failed:', deviceError);
      if (!quiet) {
        setOfficeDeviceMessage(`Could not load office devices: ${deviceError.message}`);
        setOfficeDevicesLoading(false);
      }
      return;
    }

    setOfficeDevices(data || []);
    if (!quiet) setOfficeDevicesLoading(false);
  }, [supabaseClient, userRole]);
  ...
  const fetchQuotes = useCallback(
    async ({ quiet = false } = {}) => {
      ...
      if (!profileOffice || profileOffice === 'Unknown') {
      ...
      for (let index = 0; index < quoteIds.length; index += 200) {
        ...
        if (quoteError) {
          console.error('[SupervisorQuoteOperations] completeness query failed:', quoteError);
        } else {
          supplementalRows.push(...(quoteData || []));
        }

        if (eventsError) {
          console.error('[SupervisorQuoteOperations] quote events query failed:', eventsError);
        } else {
          eventRows.push(...(eventsData || []));
        }

        if (workflowError) {
          console.error('[SupervisorQuoteOperations] 3 Yes workflow query failed:', workflowError);
        } else {
    ...
    [timeView, selectedDate, month, supabaseClient, profileOffice]
  );

  const refreshAll = useCallback(async () => {
    await Promise.all([fetchQuotes(), fetchOfficeSettings(), fetchOfficeDevices()]);
  }, [fetchQuotes, fetchOfficeSettings, fetchOfficeDevices]);
  ...
  useEffect(() => {
    // Realtime is primary; this quiet fallback catches missed device events.
    const id = setInterval(() => fetchOfficeDevices({ quiet: true }), 15000);
  ...
  useEffect(() => {
    ...
    const channel = supabaseClient
      .channel(`supervisor-quote-operations-${profileOffice}`)
  ...
  }, [fetchQuotes, fetchOfficeDevices, supabaseClient, profileOffice]);
  ...
  const agents = useMemo(() => {
    ...
    customerGroups.forEach((group) => {
      if (group.office !== profileOffice) return;
      if (group.agentName) values.add(group.agentName);
    });
    return [...values].sort((a, b) => a.localeCompare(b));
  }, [customerGroups, profileOffice]);

  const scopeGroups = useMemo(() => {
    const needle = search.trim().toLowerCase();

    return customerGroups.filter((group) => {
      if (group.office !== profileOffice) return false;
  ...
  }, [customerGroups, profileOffice, agentFilter, search]);
  ...
  const myQuoteNames = useMemo(
    ...
    [profile?.csr_name, profile?.turborater_agent_name, profile?.full_name]
  );

  const myLiveGroups = useMemo(
    () => liveGroups.filter((group) => {
      if (group.office !== profileOffice) return false;

      const latest = group.quotes?.[0] || group.latest || {};
      const capturedName = normalizePersonName(
        latest.captured_agent_name ||
        latest.csr_name ||
        group.agentName
      );
      const quoteEmail = cleanStr(latest.agent_email).toLowerCase();
      const profileEmail = cleanStr(profile?.email).toLowerCase();

      return (
        (profileEmail && quoteEmail && profileEmail === quoteEmail) ||
        (capturedName && myQuoteNames.has(capturedName))
      );
    }),
    [liveGroups, profileOffice, profile?.email, myQuoteNames]
  );

  const myActiveGroup = myLiveGroups[0] || null;
  ...
  const paymentQuoteGroups = useMemo(
    () => existingClientQuoteGroups.filter((group) => group.existingClientReason === 'rewrite_payment'),
  ...
  const paymentClosedGroups = useMemo(
    () => existingClientClosedGroups.filter((group) => group.existingClientReason === 'rewrite_payment'),
  ...
  const scopeLabel = useMemo(() => {
    const region = profileRegion || officeRegions[profileOffice] || 'Region not assigned';
    return `${profileOffice} · ${region}`;
  }, [profileOffice, profileRegion, officeRegions]);

  const officeLogAccessCount = useMemo(
    () => officeDevices.filter((row) =>
      row.office_log_access === true &&
  ...
  const pendingAdminDeviceCount = useMemo(
    () => officeDevices.filter((row) =>
      row.approval_status !== 'approved' || row.is_active !== true
    ).length,
    [officeDevices]
  );

  const setOfficeLogAccess = async (row, allowAccess) => {
    if (!row?.id) return;
    setOfficeDeviceMessage('');

    const { error: accessError } = await supabaseClient.rpc(
      'set_supervisor_office_log_access',
      {
        p_device_id: row.id,
        p_allow_access: Boolean(allowAccess),
      }
    );

    if (accessError) {
      console.error('[SupervisorQuoteOperations] office log access update failed:', accessError);
      setOfficeDeviceMessage(`Could not update office log access: ${accessError.message}`);
      return;
    }

    await fetchOfficeDevices({ quiet: true });
    setOfficeDeviceMessage(
      allowAccess
        ? `${row.device_name || 'Device'} can now view the ${profileOffice} office log.`
        : `${row.device_name || 'Device'} office log access removed.`
    );
  };
  ...
  const saveCustomerOutcome = async (event) => {
    ...
    if (agentEntry.status === 'not_closed' && agentEntry.reason === 'did_not_rewrite_current_carrier' && !agentEntry.notes.trim()) {
      setSaveMessage('Explain why the customer stayed with the current carrier before saving.');
      return;
    }
    ...
    const payload = {
      ...
      existing_client_reason: agentEntry.business_type === 'existing_client'
        ...
        : null,
      not_closed_type: agentEntry.status === 'not_closed' ? agentEntry.reason || null : null,
      not_closed_explanation: agentEntry.status === 'not_closed' && agentEntry.reason === 'did_not_rewrite_current_carrier'
    ...
    if (saveError) {
      console.error('[SupervisorQuoteOperations] workflow save failed:', saveError);
    ...
    if (overrideHistoryError) {
      console.error('[SupervisorQuoteOperations] override history save failed:', overrideHistoryError);
    ...
    if (agentEntry.notes.trim()) {
      ...
      if (noteError) {
        console.error('[SupervisorQuoteOperations] internal note save failed:', noteError);
  ...
  if (profile && userRole && userRole !== 'supervisor') {
    return (
      <div className={styles.page}>
        <div className={styles.errorBanner}>
          This page is for supervisor accounts.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          <p className={styles.eyebrow}>Supervisor / Office View</p>
          <h1>Quote Operations</h1>
          <p className={styles.subtitle}>
            Manage your office's live quotes, coach the 3 Yes process, recover lost opportunities, and confirm closed business.
          </p>
        </div>
      ...
      <section className={`${styles.scopeBar} ${styles.supervisorScopeBar}`}>
        <div className={styles.scopeTitle}>
          <span>Viewing Office</span>
      ...
      <section className={styles.statsGrid}>
        <DashboardStat
          label="Quoting Now"
          value={stats.live}
          note={`${stats.activeAgents} active agents · ${profileOffice}`}
          tone="live"
        />
        <DashboardStat
      ...
      {activeTab === 'live' && myActiveGroup && (
        <div className={styles.myActiveQuoteSection}>
          <div className={styles.myActiveQuoteHeading}>
            <div>
              <span>YOUR QUOTE</span>
              <strong>Complete the 3 Yes workflow while you quote.</strong>
            </div>
            {myLiveGroups.length > 1 && (
              <span>{myLiveGroups.length} of your quotes are currently live</span>
            )}
          </div>
          ...
          <QuoteSalesFlow
            ...
            onViewDetails={() => openCustomer(myActiveGroup)}
          />
        </div>
      )}
        ...
        <div className={styles.bucketSplitStack}>
          ...
          {unclassifiedExistingQuoteGroups.length > 0 && (
            <QuoteBucketView
              ...
              description="Existing-client re-writes that still need one of the four re-write reasons assigned."
      ...
      {activeTab === 'closed' && (
        <div className={styles.bucketSplitStack}>
          ...
          <QuoteBucketView
            ...
            description="Closed existing-client re-write deals that originated from a payment review."
          ...
          {unclassifiedExistingClosedGroups.length > 0 && (
            <QuoteBucketView
              ...
              description="Closed existing-client re-writes that still need one of the four re-write reasons assigned."
      ...
      {activeTab === 'office_access' && (
        <SupervisorOfficeAccess
          office={profileOffice}
          region={profileRegion || officeRegions[profileOffice] || 'Unassigned'}
          rows={officeDevices}
          loading={officeDevicesLoading}
          message={officeDeviceMessage}
          activeCount={officeLogAccessCount}
          pendingAdminCount={pendingAdminDeviceCount}
          onToggleAccess={setOfficeLogAccess}
          onRefresh={() => fetchOfficeDevices()}
        />
      )}
                    ...
                    return (
                      <article key={group.key} className={`${styles.liveDealCard} ${styles[`liveDealCard_${stage.tone}`] || ''}`}>
                        <div className={styles.liveDealTop}>
                          <div>
                            <span className={styles.liveNowLabel}>● LIVE · {elapsedLabel(quote.started_at || group.lastActivityAt, now)}</span>
                            <h3>{group.customerName}</h3>
                            <p>{group.agentName} · {group.office}</p>
                            <div className={styles.liveContextLine}>
                              <span>{contactMethodLabel(quote)}</span>
                              <span>·</span>
                              <span>{quoteBusinessTypeLabel(quote)}</span>
                              {inferQuoteBusinessType(quote) === 'existing_client' && (
                                <>
                                  <span>·</span>
                                  <strong>{existingClientReasonLabel(quote)}</strong>
                                </>
                              )}
                              {group.reopenedLostDeal && (
                                <>
                                  <span>·</span>
                                  <strong>RE-OPENED LOST DEAL</strong>
                                </>
                              )}
                            </div>
                          </div>
                          ...
                          {quote.office_mismatch && (
                            <span
                              className={styles.officeMismatchBadge}
                              title={`TurboRater office ${quote.turborater_office || 'unknown'} differs from trusted-device office ${quote.office || group.office}`}
                            >
                              Office Mismatch
                            </span>
                          )}
          ...
          <section className={styles.drawerSection}>
            <div className={styles.drawerSectionHeading}>
              <div>
                ...
                {latest.office_mismatch && (
                  <div className={styles.officeMismatchNotice}>
                    <strong>Office mismatch detected</strong>
                    <span>
                      TurboRater showed {latest.turborater_office || 'an unknown office'}, but this quote is assigned to
                      {' '}{latest.office || group.office} from the trusted extension device.
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className={styles.quoteDetailGrid}>
              ...
              <BridgeField label="Operational Office" value={latest.office || group.office} />
              <BridgeField
                label="Office Source"
                value={
                  latest.office_source === 'trusted_device'
                    ? 'Trusted Device'
                    : latest.office_source === 'legacy_turborater'
                      ? 'Legacy TurboRater'
                      : latest.office_source || '—'
                }
              />
              <BridgeField label="TurboRater Office" value={latest.turborater_office || '—'} />
              <BridgeField
                label="Office Match"
                value={
                  latest.office_mismatch
                    ? `⚠ Mismatch — routed to ${latest.office || group.office}`
                    : latest.office_source === 'trusted_device'
                      ? '✓ Matches trusted device'
                      : '—'
                }
              />
              <BridgeField label="Contact Method" value={contactMethodLabel(latest)} />
                ...
                {agentEntry.status === 'not_closed' && (
                  <label>
                    ...
                    <select value={agentEntry.reason} onChange={(e) => setAgentEntry((current) => ({ ...current, reason: e.target.value }))}>
                      ...
                      {NOT_CLOSED_REASONS
                        .filter(([value]) => value !== 'did_not_rewrite_current_carrier')
                        .map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                      {agentEntry.business_type === 'existing_client' && agentEntry.existing_client_reason && (
                        <option value="did_not_rewrite_current_carrier">Did Not RW - Stayed With Current Carrier</option>
                      )}
                    </select>
                  </label>
                )}
                ...
                {group.quotes.slice(1).map((quote) => (
                  <details key={quote.id} className={styles.bridgeItem}>
                    ...
                    <div className={styles.bridgeGrid}>
                      ...
                      <BridgeField label="Contact Method" value={contactMethodLabel(quote)} />
                      <BridgeField label="Quote Type" value={quoteBusinessTypeLabel(quote)} />
                      <BridgeField label="Operational Office" value={quote.office || group.office} />
                      <BridgeField label="TurboRater Office" value={quote.turborater_office || '—'} />
...
function SupervisorOfficeAccess({
  office,
  region,
  rows,
  loading,
  ...
  const sortedRows = [...rows].sort((a, b) => {
    const aReady = a.approval_status === 'approved' && a.is_active === true ? 0 : 1;
    const bReady = b.approval_status === 'approved' && b.is_active === true ? 0 : 1;
  ...
  return (
    <section className={styles.section}>
      <div className={styles.sectionHeading}>
        <div>
          <span className={styles.sectionKicker}>Office device access</span>
          <h2>{office} Office Log Access</h2>
          <p>
            Choose which Admin-approved computers can open this office's quote log.
            Device trust and activation remain controlled by Admin.
          </p>
        </div>

        <div className={styles.officeAccessSummary}>
          <span className={styles.officeAccessActive}>{activeCount} with log access</span>
          {pendingAdminCount > 0 && (
            <span className={styles.officeAccessPending}>{pendingAdminCount} waiting on Admin</span>
          )}
        </div>
      </div>

      <div className={styles.officeAccessNotice}>
        <strong>{office} · {region}</strong>
        <span>
          Supervisors cannot approve, reject, activate, disable, or move trusted devices.
          You can only allow or remove Office Log Access after Admin has approved the device.
        </span>
      </div>

      {message && <div className={styles.deviceMessage}>{message}</div>}

      <div className={styles.officeAccessToolbar}>
        <div>
          <span>Office computers</span>
      ...
      {sortedRows.length === 0 ? (
        <div className={styles.emptyState}>
          <strong>No computers registered for this office</strong>
          <span>
            Once a computer registers for {office}, it will appear here. Admin must fully approve it
            before a supervisor can allow Office Log Access.
          </span>
        </div>
      ) : (
        <div className={styles.officeAccessGrid}>
          {sortedRows.map((row) => {
            const adminApproved = row.approval_status === 'approved' && row.is_active === true;
            const hasAccess = adminApproved && row.office_log_access === true;

            return (
              <article key={row.id} className={styles.officeAccessCard}>
                <div className={styles.officeAccessCardTop}>
                  <div>
                    <h3>{row.device_name || 'Unnamed computer'}</h3>
                    <p>{row.requested_by_name || row.requested_by_email || 'Registration request'}</p>
                  </div>

                  <span
                    className={`${styles.officeAccessStatus} ${
                      hasAccess
                        ? styles.officeAccessStatusAllowed
                        : adminApproved
                          ? styles.officeAccessStatusReady
                          : styles.officeAccessStatusWaiting
                    }`}
                  >
                    {hasAccess ? 'Office Log Allowed' : adminApproved ? 'Admin Approved' : 'Waiting for Admin'}
                  </span>
                </div>

                <div className={styles.officeAccessMeta}>
                  <div>
                  ...
                  <div>
                    ...
                    <strong>{hasAccess ? 'Allowed' : 'No Access'}</strong>
                  </div>
                  <div>
                    <span>Last Used</span>
                    <strong>{row.last_used_at ? formatDateTime(row.last_used_at) : 'Never'}</strong>
                  </div>
                  <div>
                ...
                <div className={styles.officeAccessActions}>
                  {adminApproved ? (
                    hasAccess ? (
                      <button
                        type="button"
                        className={styles.dangerTextButton}
                        onClick={() => onToggleAccess(row, false)}
                      >
                        Remove Office Log Access
                      </button>
                    ) : (
                      <button
                        type="button"
                        className={styles.primaryButton}
                        onClick={() => onToggleAccess(row, true)}
                      >
                        Allow Office Log
                      </button>
                    )
