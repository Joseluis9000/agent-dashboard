...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const res = await fetch(`https://script.google.com/macros/s/AKfycbx5i4AZEB-k2O0Dlw-MqDYtsvFN5Cap2hjDa3Rxjs7OG3UGvlmBTv8-gU4FGUbenq-K/exec?email=${userEmail}`);
                if (!res.ok) {
                ...
                if (data.success) {
                    // ✅ FIX: Get the first item from the array
                    setCurrentCommission(data.commissionData && data.commissionData.length > 0 ? data.commissionData[0] : null);
                    setCommissionHistory(data.commissionHistoryData || []);
                    // Set the agent's first name for the welcome message
                    setAgentName(data.commissionData?.[0]?.agentname?.split(" ")[0] || 'Agent');
