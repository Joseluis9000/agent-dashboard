...
/** Violation categories for the new dropdown */
...
const calcFee = (violation_type, violation_category) => {
  ...
  if (violation_type === 'Scanning Violation') {
    ...
    return 0; // default for other categories under Scanning
  }
  return 0;
};
...
const EnterViolation = () => {
  ...
  const initialRow = {
    ...
    office_code: '',
    region: '',
    violation_type: 'AR Violation', // renamed default
    violation_category: '', // NEW FIELD
    client_name: '',
    reference_id: '',
    variance_amount: 0,
    fee_amount: 10,
  ...
  useEffect(() => {
    if (violationToEdit) {
      // Map legacy "AR Shortage" to new "AR Violation" and seed category if missing
      setRows([{
        ...violationToEdit,
        id: violationToEdit.id,
        violation_type: violationToEdit.violation_type === 'AR Shortage'
          ? 'AR Violation'
  ...
  const handleRowChange = (index, field, value) => {
    ...
    if (field === 'violation_type' || field === 'violation_category') {
      const vt = field === 'violation_type' ? value : newRows[index].violation_type;
      const vc = field === 'violation_category' ? value : newRows[index].violation_category;
      newRows[index].fee_amount = calcFee(vt, vc);

      // Clear variance if not AR
      if (vt !== 'AR Violation') newRows[index].variance_amount = 0;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (violationToEdit) {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          violation_category: rowToUpdate.violation_category, // NEW
          week_start_date: week.start,
          week_end_date: week.end,
          variance_amount: rowToUpdate.violation_type === 'AR Violation' ? rowToUpdate.variance_amount : 0,
          fee_amount: calcFee(rowToUpdate.violation_type, rowToUpdate.violation_category),
    ...
    for (const row of rows) {
      ...
      violationsToInsert.push({
        ...
        violation_category: row.violation_category, // NEW
        week_start_date: week.start,
        week_end_date: week.end,
        variance_amount: row.violation_type === 'AR Violation' ? row.variance_amount : 0,
    ...
    <div className={styles.container}>
      ...
      <form onSubmit={handleSubmit} className={styles.formCard}>
        ...
        <div className={styles.entryGrid}>
          ...
          <div className={styles.gridHeader}>Violation Category</div>
          ...
          {rows.map((row, index) => (
            <React.Fragment key={row.id}>
              ...
              <input
                ...
                onChange={(e) => handleRowChange(index, 'reference_id', e.target.value)}
                required
              />
