...
// Reusable table for a violation group
          ...
          {violations.length > 0 ? (
            violations.map((v) => (
              <tr key={v.id}>
                ...
                <td>{v.office_code}</td>
                <td>{v.region || 'N/A'}</td>
                <td>{v.violation_category || '—'}</td> {/* NEW */}
                <td>
                  {v.client_name}
                ...
                <td>
                  <select
                    ...
                    className={`${styles.statusSelect} ${styles[(v.status || 'Pending').toLowerCase()]}`}
                  >
            ...
            <tr>
              {/* 12 headers -> span 12 */}
              <td colSpan="12">No violations for this period.</td>
            </tr>
          )}
        </tbody>
      </table>
...
const ManageViolations = () => {
  ...
  // Stable fetch function so hooks deps are satisfied
  ...
  // Group rows (treat legacy "AR Shortage" as AR Violation)
  const isAR = (vt) => vt === 'AR Violation' || vt === 'AR Shortage';
  const arViolations = violations.filter((v) => isAR(v.violation_type));
    ...
    <div className={styles.container}>
      ...
      {loading ? (
        ...
        <div className={styles.tablesGrid}>
          <ViolationTable
            title="AR Violations"   // renamed from "AR Shortages"
